...
namespace Sms.Protocol.Cmpp2
{
	...
	public struct CmppDeliver : IMessage
	{
		#region 消息体
		/// <summary>
		...
		public CmppDeliver FromBytes0(byte[] array)
		{
			FromBytes(array);
			return this;
		}
		...
		public CmppReport GetReport()
		{
			...
			//LogHelper.log.Debug("GetReport:" + bytes.Length);
			//LogHelper.log.Debug("GetReport1:" + MsgContent);
		...
		{
			string msgContent = RegisteredDelivery == 1 ? GetReport().ToString() : this.MsgContent;
			return "DestId: " + this.DestId + "," +
				//"LinkId: " + this.LinkId + "," +
				"MsgContent: " + msgContent + "," +
				"MsgFmt: " + this.MsgFmt + "," +
				"MsgId: " + this.MsgId + "," +
				"MsgLength: " + this.MsgLength + "," +
				"RegisteredDelivery: " + this.RegisteredDelivery + "," +
				"Reserved: " + this.Reserved + "," +
				"ServiceId: " + this.ServiceId + "," +
				"SrcTerminalId: " + this.SrcTerminalId + "," +
				"SrcTerminalType: " + this.SrcTerminalType + "," +
				"TPPId: " + this.TPPId + "," +
				"TPUdhi: " + this.TPUdhi + ";";
		}
		#endregion
		...
		public byte[] ToBytes()
		{
			var buffer = new List<byte>(CmppConstants.HeaderSize + MsgLength + 73);
			buffer.AddRange(Convert.ToBytes(MsgId));//8
			buffer.AddRange(Convert.ToBytes(DestId, CmppConstants.Encoding.ASCII, 21));
			buffer.AddRange(Convert.ToBytes(ServiceId, CmppConstants.Encoding.ASCII, 10));
			buffer.Add(TPPId);
			buffer.Add(TPUdhi);
			buffer.Add(MsgFmt);
			buffer.AddRange(Convert.ToBytes(SrcTerminalId, CmppConstants.Encoding.ASCII, 21));
			buffer.Add(RegisteredDelivery);
			buffer.Add(MsgLength);
			byte[] content = RegisteredDelivery == 0
				? Convert.ToBytes(MsgContent, MsgFmt)
				: GetReport().ToBytes();
			if (MsgLength != 0 || content != null)
			{
				buffer.AddRange(content);
			}
			buffer.AddRange(Convert.ToBytes(Reserved, CmppConstants.Encoding.ASCII, 8));
		...
		public void FromBytes(byte[] buffer)
		{
			...
			MsgId = Convert.ToUInt64(buffer, position);
			position = position + 8;

			DestId = Convert.ToString(buffer, position, 21, CmppEncoding.ASCII).TrimEnd('\0');
			position = position + 21;

			ServiceId = Convert.ToString(buffer, position, 10, CmppEncoding.ASCII).TrimEnd('\0');
			position = position + 10;
			...
			SrcTerminalId = Convert.ToString(buffer, position, 21, CmppEncoding.ASCII).TrimEnd('\0');
			position = position + 21;
			...
			position = position + MsgLength;
			Reserved = Convert.ToString(buffer, position, 8, CmppEncoding.ASCII).TrimEnd('\0');
		}
